/*
	...
	This program is a basic web server that implements forking
*/
...
void generateLog(char add[], string request, string statusCode, int numBytes)
{
	...
	out.open("access_logs2.txt", ios::app);
	out << log;
...
/*does all of the work that was previously done in the main method. added as a function so that the process of forking would be simplified*/
void do_work(int fd, struct sockaddr_in sa)
{
	string statusCode = "";
	char buf[10240];
	int rlen, numBytes;
